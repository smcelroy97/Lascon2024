 ...
 *
 *  Generated from NESTML at time: 2024-01-22 18:57:28.422672
**/
...
extern "C" inline int adex_gamma_E_ml_dynamics(double __time, const double ode_state[], double f[], void* pnode)
{
  ...
  f[State_::V_m] = node.P_.E_e * (1.0 * ode_state[State_::g_NMDA__X__NMDA] / (0.280112044817927 * node.P_.C_m * node.P_.Mg * std::exp((-0.062) * std::min(ode_state[State_::V_m], node.P_.V_peak)) + node.P_.C_m) + 1.0 * ode_state[State_::g_AMPA__X__AMPA] / node.P_.C_m) + node.P_.g_L * (node.P_.E_L / node.P_.C_m - std::min(ode_state[State_::V_m], node.P_.V_peak) / node.P_.C_m) - 1.0 * ode_state[State_::g_NMDA__X__NMDA] * std::min(ode_state[State_::V_m], node.P_.V_peak) / (0.280112044817927 * node.P_.C_m * node.P_.Mg * std::exp((-0.062) * std::min(ode_state[State_::V_m], node.P_.V_peak)) + node.P_.C_m) + node.P_.Delta_T * node.P_.g_L * std::exp(((-node.P_.V_th) + std::min(ode_state[State_::V_m], node.P_.V_peak)) / node.P_.Delta_T) / node.P_.C_m + 1.0 * node.P_.E_i * ode_state[State_::g_GABA__X__GABA] / node.P_.C_m + (node.P_.I_e - 1.0 * ode_state[State_::g_AMPA__X__AMPA] * std::min(ode_state[State_::V_m], node.P_.V_peak) - 1.0 * ode_state[State_::g_GABA__X__GABA] * std::min(ode_state[State_::V_m], node.P_.V_peak) - ode_state[State_::w]) / node.P_.C_m;
